...
Aby interfejsy by≥y interpretowane jako serwisy, muszπ byÊ one oznaczone adnotacjπ \texttt{Service}. Serwis artyku≥Ûw (listing~\ref{lst:serwis-artykulow}) korzysta z repozytoriÛw operujπcych na artyku≥ach, komentarzach, uøytkownikach oraz technologiach. W serwisach zaimplementowana jest obs≥uga wyjπtkÛw i np. w przypadku nie podania id artyku≥u w metodzie \texttt{getArticleById}, zostanie rzucony wyjπtek z komunikatem ,,Nie istnieje artyku≥ o takim id''. Wyjπtki rzucane przez serwisy sπ pÛüniej obs≥ugiwane przez kontrolery.
