...
Uøyto nastÍpujπcych technologii: \texttt{Java}~\cite{Java}, \texttt{Spring}~\cite{Spring}, \texttt{React}~\cite{React}, \texttt{Typescript}~\cite{TypeScript}, 
\texttt{Redux}~\cite{Redux}, \texttt{MUI}~\cite{MUI}, \texttt{PostgreSQL}~\cite{PostgreSQL}, \texttt{MongoDB}~\cite{MongoDB}, \texttt{NGINX}~\cite{NGINX}, 
\texttt{Keycloak}~\cite{Keycloak}.
...
\begin{enumerate}
	\item \texttt{LaTeX}~\cite{LaTeX} - napisanie dokumentacji,
	\item \texttt{Visual Paradigm}~\cite{Visual} - sporzπdzenie diagramÛw,
	\item \texttt{Figma}~\cite{Figma} - zaprojektowanie interfejsu uøytkownika,
	\item \texttt{pgAdmin}~\cite{pgAdmin} - zarzπdzanie bazπ PostgreSQL,
	\item \texttt{MongoDBCompass}~\cite{MongoDBCompass} - zarzπdzanie bazπ MongoDB,
	\item \texttt{Visual Studio Code}~\cite{Code} - úrodowisko programistyczne dla frontendu,
	\item \texttt{Itellij}~\cite{Itellij} - úrodowisko programistyczne dla backendu,
	\item \texttt{Postman}~\cite{Postman} - testowanie wysy≥ania øπdaÒ do backendu,
	\item \texttt{REST-assured}~\cite{REST-assured} - testy integracyjne backendu,
	\item \texttt{Swagger}~\cite{Swagger} - dokumentacja endpointÛw backendu,
	\item \texttt{Docker}~\cite{Docker} - stworzenie i umieszczenie kontenerÛw aplikacji w rejestrze kontenerÛw,
	\item \texttt{MongoDB Cloud}~\cite{MongoDBCloud} - hostowanie bazy MongoDB,
	\item \texttt{Azure}~\cite{Azure} - hostowanie ca≥ego systemu oprÛcz bazy MongoDB.
\end{enumerate}
...
\begin{figure}[H]
	...
	\caption*{èrÛd≥o: Opracowanie w≥asne - zrzut ekranu z programu Swagger}
\end{figure}
